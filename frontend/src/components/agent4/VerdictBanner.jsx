      ...
      <div className="p-6">
        ...
        <div className="flex items-start justify-between gap-4 mb-5">
          <div>
            ...
            <h2 className="text-2xl font-bold text-white">Chief Proposal Review Officer</h2>
            <p className="text-xs text-gray-500 mt-1">Aggregated from all three specialist agents via AWS Bedrock</p>
          </div>
          <div className={`flex items-center gap-2 px-4 py-2 rounded-full border font-bold text-sm ${cfg.badge}`}>
            <span>{cfg.icon}</span>
            <span>{verdict}</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-6 mb-5">
          ...
          <div className="relative flex-shrink-0" style={{ width: 80, height: 80 }}>
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className="text-xl font-bold text-white leading-none">{overallScore?.toFixed(1)}</span>
        ...
        <div className="grid grid-cols-3 gap-3 mb-5">
          {agentPills.map(({ label, score, colour }) => (
            <div key={label} className="bg-gray-900 border border-gray-800 rounded-xl p-3 text-center">
              <p className={`text-lg font-bold ${colour}`}>{score?.toFixed(1)}</p>
