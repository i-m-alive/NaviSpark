...
function buildGuideText(guide, skipped, summary) {
  const lines = []
  const hr    = '='.repeat(54)
  const sep   = '-'.repeat(54)

  lines.push('PPT EDIT GUIDE')
  ...
  groups.forEach(({ label, items }) => {
    ...
    items.forEach(item => {
      lines.push(`Edit #${item.change_number} — Slide ${item.slide_number}: ${item.slide_title}`)
      ...
      lines.push(`Location: ${item.shape_name || '(body text area)'}`)
      lines.push('')
      ...
      if (item.action === 'replace_text') {
        if (item.find_text) {
          lines.push('FIND THIS TEXT IN YOUR PPT:')
...
function downloadGuide(guide, skipped, summary) {
  const text = buildGuideText(guide, skipped, summary)
  const blob = new Blob([text], { type: 'text/plain;charset=utf-8' })
  const url  = URL.createObjectURL(blob)
  const a    = document.createElement('a')
  a.href     = url
  a.download = 'ppt_edit_guide.txt'
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0">
          <button
            onClick={() => downloadGuide(guide, skipped, summary)}
