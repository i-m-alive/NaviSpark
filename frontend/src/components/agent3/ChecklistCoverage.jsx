...
export default function ChecklistCoverage({ checklist }) {
    ...
    <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs font-mono text-gray-500">{checklist.length} items</span>
        </div>
        <div className="flex items-center gap-3">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-800 bg-gray-950">
                ...
                <th className="text-left px-3 py-2.5 text-xs font-mono text-gray-500">Topic</th>
                <th className="text-center px-3 py-2.5 text-xs font-mono text-gray-500 w-16">Skill</th>
              ...
              {checklist.map((item, idx) => {
                ...
                return (
                  <tr key={item.id || idx} className={clsx('border-b border-gray-800/50 last:border-0', cfg.rowClass)}>
                    ...
                    <td className="px-3 py-3 text-gray-200 font-medium text-xs">{item.topic || '—'}</td>
                    <td className="px-3 py-3 text-center text-xs font-mono text-teal-400">{item.skill || '—'}</td>
