...
async def run_agent1(
    ...
    if session.get("status") not in ("ready", "agent1_complete", "agents_complete", "complete"):
        raise HTTPException(
            status_code=400,
...
async def run_agent2(
    ...
    if session.get("status") not in ("ready", "agent1_complete", "agent2_complete", "agents_complete", "complete"):
        raise HTTPException(
            status_code=400,
            detail=f"Session not ready for analysis. Current status: {session.get('status')}.",
        )
